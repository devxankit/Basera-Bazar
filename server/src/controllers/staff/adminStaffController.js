...
const updateTeamLeader = async (req, res) => {
  try {
    ...
    const tl = await TeamLeader.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true });
    if (!tl) return res.status(404).json({ success: false, message: 'Team Leader not found.' });
...
const approveOfficeStaff = async (req, res) => {
  try {
    const os = await OfficeStaff.findByIdAndUpdate(req.params.id, { onboarding_status: 'approved' }, { new: true });
    if (!os) return res.status(404).json({ success: false, message: 'Office Staff not found.' });
...
const rejectOfficeStaff = async (req, res) => {
  try {
    const { reason } = req.body;
    if (!reason) return res.status(400).json({ success: false, message: 'Rejection reason is required.' });
    const os = await OfficeStaff.findByIdAndUpdate(req.params.id, { onboarding_status: 'rejected', deactivation_reason: reason }, { new: true });
    if (!os) return res.status(404).json({ success: false, message: 'Office Staff not found.' });
...
const reassignOfficeStaff = async (req, res) => {
  try {
    ...
    const os = await OfficeStaff.findByIdAndUpdate(req.params.id, { team_leader_id: new_team_leader_id }, { new: true });
    if (!os) return res.status(404).json({ success: false, message: 'Office Staff not found.' });
...
const assignExecutiveToTL = async (req, res) => {
  try {
    ...
    if (!tl) return res.status(404).json({ success: false, message: 'Team Leader not found.' });
    const exec = await Executive.findByIdAndUpdate(req.params.id, { team_leader_id }, { new: true });
