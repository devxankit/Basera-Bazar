...
export default function OfficeStaffTargets() {
  const [targets, setTargets] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get('/office-staff/targets')
  ...
  return (
    <div className="p-4 max-w-lg mx-auto space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500">Current and historical performance targets</p>
      </div>
        ...
        return (
          <div key={t._id} className="bg-white border border-slate-200 rounded-lg p-4 space-y-3">
            <div className="flex items-start justify-between">
              ...
              <span className={`px-2 py-0.5 rounded text-xs font-bold ${t.is_active ? 'bg-teal-100 text-teal-700' : 'bg-slate-100 text-slate-500'}`}>
                {t.is_active ? 'Active' : 'Closed'}
              </span>
            </div>
            <div>
              <div className="flex justify-between text-xs text-slate-500 mb-1">
                <span>Achieved: {t.achieved_value ?? 0} / {t.target_value}</span>
            ...
            <div className="flex justify-between text-xs">
              <span className="text-slate-500">
                Incentive: <strong className="text-teal-700">{t.incentive_type === 'percentage' ? `${t.incentive_rate}%` : `₹${t.incentive_rate}/unit`}</strong>
              </span>
