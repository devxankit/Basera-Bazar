...
export default function AdminUserDetails() {
  ...
  const { data: propertyListings = [] } = useQuery({
    ...
    queryFn: () =>
      api
        .get(`/admin/listings/property?partner_id=${id}&limit=50`)
        .then((r) => r.data.data || []),
    enabled: !!user,
    staleTime: 5 * 60 * 1000,
  });
  ...
  const { data: serviceListings = [] } = useQuery({
    ...
    queryFn: () =>
      api
        .get(`/admin/listings/service?partner_id=${id}&limit=50`)
        .then((r) => r.data.data || []),
    enabled: !!user,
    staleTime: 5 * 60 * 1000,
  });
  ...
  const { data: productListings = [] } = useQuery({
    ...
    queryFn: () =>
      api
        .get(`/admin/listings/product?partner_id=${id}&limit=50`)
        .then((r) => r.data.data || []),
    enabled: !!user,
    staleTime: 5 * 60 * 1000,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (updateData) =>
      api.put(`/admin/users/${id}`, updateData).then((r) => r.data),
    onSuccess: () =>
  ...
  if (error || !user)
    return (
      <div className="p-12 text-center bg-white border border-slate-200 rounded-2xl m-8 shadow-sm">
        ...
        <h2 className="text-xl font-semibold text-slate-900 uppercase tracking-tight">
          {error || "User Not Found"}
        </h2>
        <button
          onClick={() => navigate("/admin/users")}
          className="mt-6 px-6 py-2 bg-slate-900 text-white text-xs font-semibold uppercase tracking-widest rounded-xl hover:bg-indigo-600 transition-all active:scale-95">
          Back to Users
        </button>
      </div>
    );
  ...
  const displayRole =
    user.displayRole || (user.role === "user" ? "Customer" : user.role);
      ...
      <div className="max-w-400 mx-auto px-8 space-y-8 mt-6">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-3">
                    <h2 className="text-3xl font-semibold text-slate-900 tracking-tight leading-none uppercase">
                      {user.name}
                    </h2>
                    <div className="flex gap-1.5">
                      {displayRoles.map((role, idx) => (
                        <span
                          key={idx}
                          className="px-3 py-1 bg-slate-100 text-slate-600 border border-slate-200 rounded-lg text-[11px] font-semibold uppercase tracking-widest">
                          {role}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] font-semibold text-indigo-600 uppercase tracking-widest bg-indigo-50 px-2.5 py-1 rounded border border-indigo-100">
                ...
                {showOptions && (
                  <>
                    ...
                    <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 p-2 z-50">
                      <button
                        onClick={() =>
                          navigate(`/admin/users/subscriptions/${user._id}`)
                        }
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            {
              label: "Products",
              value: user.stats?.products || 0,
              icon: Package,
              color: "bg-orange-50 text-orange-600",
            },
            {
              label: "Leads",
              value: user.stats?.leads || 0,
              icon: TrendingUp,
            ...
            <div
              ...
              <div>
                ...
                <p className="text-2xl font-semibold text-slate-900 tracking-tighter">
                  {stat.value}
                </p>
              </div>
            </div>
              ...
              <div className="p-8">
                ...
                {isPartner && user.active_subscription && (
                  <div className="mt-8 pt-8 border-t border-slate-200">
                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest block mb-4">
                      Active Subscription
                    </label>
                      ...
                      {[
                        {
                          label: "Current Plan",
                          value:
                            user.active_subscription.plan_snapshot?.name ||
                            "Standard",
                        ...
                        },
                        {
                          label: "Listing Limit",
                          value:
                            user.active_subscription.plan_snapshot
                              ?.listings_limit === -1
                              ? "Unlimited"
                              : user.active_subscription.plan_snapshot
                                  ?.listings_limit,
            ...
            {isPartner && (
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-8 py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-semibold bg-emerald-50 text-emerald-600 px-2 py-1 rounded border border-emerald-100 uppercase tracking-widest">
                    Verified Assets
                  </span>
                </div>
  ...
  const groups = [
    {
      ...
      label: "Properties",
      Icon: Building2,
      headerCls: "text-indigo-600",
      badgeCls: "bg-indigo-50 text-indigo-700",
      items: properties,
      pathPrefix: "/admin/properties/view",
    },
    {
      key: "services",
      label: "Services",
      ...
      items: services,
      pathPrefix: "/admin/services/view",
    },
    {
      key: "products",
      label: "Mandi Products",
      Icon: Package,
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-8 py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
        ...
        <span className="text-[10px] font-semibold bg-indigo-50 text-indigo-600 px-2 py-1 rounded border border-indigo-100 uppercase tracking-widest">
          All Types
        </span>
      </div>
      ...
      {totalCount === 0 ? (
        <div className="p-10 text-center">
          ...
          <p className="text-sm font-semibold text-slate-500">
            This partner hasn't created any listings yet.
          </p>
        </div>
      ) : (
...
function ListingCard({ item, badgeCls, onView }) {
  ...
  const price =
    ...
    item.budget_min;
  const status =
    item.status || (item.is_active === false ? "inactive" : "active");
  const location =
