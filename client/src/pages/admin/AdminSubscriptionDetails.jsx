...
export default function AdminSubscriptionDetails() {
        ...
        <div className="space-y-3">
           <div className="flex justify-between items-end">
              <div className="flex flex-col">
                <span className="text-[14px] font-black text-slate-800 tracking-tight">{current === -1 ? '∞' : current}</span>
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{label} Available</span>
              </div>
              <div className="flex flex-col items-end">
                <span className="text-[14px] font-black text-orange-500 tracking-tight">{total === -1 ? '∞' : total}</span>
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{total === -1 ? 'Unlimited' : 'Featured Available'}</span>
              </div>
           </div>
           ...
           <div className="flex justify-center">
              <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-tighter">{current}/{total === -1 ? '∞' : total} used</span>
           </div>
        </div>
     );
  };
  ...
  const handleDownloadInvoice = () => {
    alert('Preparing your invoice for download...');
    // Real implementation would trigger a PDF download or print view
  };
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20 animate-in fade-in duration-700 text-left">
      ...
      <div className="px-8 py-6 flex items-center justify-between">
         <div className="flex flex-col">
            <h1 className="text-[18px] font-black text-slate-900 tracking-tight">Subscription Details</h1>
      ...
      <div className="max-w-[1600px] mx-auto px-8 space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-[32px] p-6 shadow-sm flex items-center justify-between group">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <h2 className="text-[15px] font-black text-slate-900 uppercase tracking-widest leading-none">Subscription Information</h2>
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5 italic">Detailed diagnostic of active partner entitlement</p>
              </div>
           </div>
           ...
           <div className="flex items-center gap-3">
              <button 
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-white border-2 border-slate-200 text-slate-600 font-black text-[10px] rounded-xl hover:bg-slate-50 transition-all uppercase tracking-widest shadow-sm"
              >
                 <ArrowLeft size={16} /> Back to Subscriptions
              </button>
              <button 
                onClick={() => navigate(`/admin/users/view/${partner_id?._id}`)}
                className="flex items-center gap-2 px-5 py-2.5 bg-white border-2 border-cyan-400 text-cyan-500 font-black text-[10px] rounded-xl hover:bg-cyan-50 transition-all uppercase tracking-widest shadow-sm"
              >
                 <User size={16} /> View User
              </button>
              <button 
                 onClick={() => navigate(`/admin/subscriptions/add-manual/${partner_id?._id}`)}
                 className="flex items-center gap-2 px-5 py-2.5 bg-[#4ADE80] text-white font-black text-[10px] rounded-xl hover:bg-emerald-500 transition-all uppercase tracking-widest shadow-lg shadow-emerald-100 border border-emerald-400/20"
              >
                 <RefreshCcw size={16} /> Renew
              </button>
              <button className="flex items-center gap-2 px-5 py-2.5 bg-white border-2 border-rose-400 text-rose-500 font-black text-[10px] rounded-xl hover:bg-rose-50 transition-all uppercase tracking-widest shadow-sm">
        ...
        <div className="grid grid-cols-12 gap-8 items-start">
           ...
           <div className="col-span-12 lg:col-span-8 space-y-8">
              ...
              <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
                 <div className="px-8 py-5 border-b border-slate-50 flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Subscription Overview</span>
                 </div>
                 <div className="p-8">
                    <div className="grid grid-cols-2 gap-y-10 gap-x-12">
                       <div className="space-y-1">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Subscription ID</p>
                          <p className="text-[14px] font-black text-slate-900 tracking-tight">#{subscription?._id?.slice(-6).toUpperCase() || 'N/A'}</p>
                       </div>
                       <div className="space-y-1 text-right lg:text-left">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Start Date</p>
                          <p className="text-[14px] font-black text-slate-900 tracking-tight italic">{formatDate(subscription.starts_at)}</p>
                       </div>
                       <div className="space-y-1">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Plan Name</p>
                          <p className="text-[14px] font-black text-slate-900 tracking-tight italic">{plan_snapshot?.name || 'Free Trail'}</p>
                       </div>
                       <div className="space-y-1 text-right lg:text-left">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">End Date</p>
                          <p className="text-[14px] font-black text-slate-900 tracking-tight italic">{formatDate(subscription.ends_at)}</p>
                       </div>
                       <div className="space-y-1">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">User</p>
                          <div className="flex items-center gap-2 mt-1">
                             <div className="w-6 h-6 rounded-full bg-indigo-600 flex items-center justify-center text-white text-[9px] font-black">
                                {partner_id?.name?.[0] || 'U'}
                             </div>
                             <span className="text-[13px] font-black text-slate-800 tracking-tight">{partner_id?.name || 'Ujjawal'}</span>
                             <span className="bg-slate-800 text-white text-[7px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded ml-1">
                       ...
                       <div className="space-y-1 text-right lg:text-left">
                          ...
                          <p className="text-[14px] font-black text-slate-900 tracking-tight italic">{plan_snapshot?.duration_days || 30} Days</p>
                       </div>
                       <div className="space-y-3">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Status</p>
                          <span className={cn(
                             "inline-block px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-[0.15em] shadow-sm",
                       ...
                       <div className="space-y-1 text-right lg:text-left">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Payment Method</p>
                          <p className="text-[14px] font-black text-slate-900 tracking-tight italic">
              ...
              <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
                 <div className="px-8 py-5 border-b border-slate-50 flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Plan Features</span>
                 </div>
                 <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center gap-3 text-emerald-500">
                       <CheckCircle2 size={16} />
                       <span className="text-[12px] font-black text-slate-600 tracking-tight italic">{plan_snapshot?.duration_days || 30} days Account Validity</span>
                    </div>
                    <div className="flex items-center gap-3 text-emerald-500">
                       <CheckCircle2 size={16} />
                       <span className="text-[12px] font-black text-slate-600 tracking-tight italic">{plan_snapshot?.listings_limit === -1 ? 'Unlimited' : plan_snapshot?.listings_limit || 0} Listings</span>
                    </div>
                    <div className="flex items-center gap-3 text-emerald-500">
                       <CheckCircle2 size={16} />
                       <span className="text-[12px] font-black text-slate-600 tracking-tight italic">{plan_snapshot?.featured_listings_limit === -1 ? 'Unlimited' : plan_snapshot?.featured_listings_limit || 0} Featured Listing</span>
                    </div>
                    <div className="flex items-center gap-3 text-emerald-500">
                       <CheckCircle2 size={16} />
                       <span className="text-[12px] font-black text-slate-600 tracking-tight italic">{plan_snapshot?.leads_limit === -1 ? 'Unlimited' : plan_snapshot?.leads_limit || 0} leads</span>
                    </div>
                    {plan_snapshot?.features?.map((f, i) => (
                      <div key={i} className="flex items-center gap-3 text-emerald-500">
                        <CheckCircle2 size={16} />
                        <span className="text-[12px] font-black text-slate-600 tracking-tight italic">{f}</span>
                      </div>
                    ))}
                 </div>
              </div>
           ...
           <div className="col-span-12 lg:col-span-4 space-y-8">
              ...
              <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
                 ...
                 <div className="p-8 flex flex-col items-center">
                    <div className="text-center mb-8">
                       <span className="text-[28px] font-black text-[#4ADE80] tracking-tight leading-none italic">₹{plan_snapshot?.price || 0}.00</span>
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Amount Paid</p>
                    </div>
              ...
              <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
                 <div className="px-8 py-5 border-b border-slate-50 flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Usage Statistics</span>
                 </div>
           ...
           <div className="col-span-12 bg-white border border-slate-200 rounded-[28px] p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm overflow-hidden relative">
              <div className="w-full md:w-[150px]">
                 <span className="text-[12px] font-black text-slate-800 uppercase tracking-tight">Time Remaining</span>
                 <p className="text-[16px] font-black text-slate-400 mt-1 italic tracking-tighter">{diffDays} days</p>
              </div>
        ...
        <div className="pt-12 flex items-center justify-between border-t border-slate-200 opacity-60">
           <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">© 2026 BaseraBazar - Real Estate & Construction Marketplace</p>
