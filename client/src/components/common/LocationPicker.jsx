...
async function forwardGeocode(query) {
  if (GMAPS_KEY) {
    try {
      ...
      if (data.status === "OK" && data.results.length > 0) {
        return data.results
          .map((item) => {
            const { city, district, state } = parseGoogleComponents(
  ...
  try {
    ...
    return results
      .map((item) => {
        const { city, district, state } = parseNominatimAddress(
          item.address || {},
        );
        return {
...
export default function LocationPicker({
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-t-4xl overflow-hidden">
      {/* Header */}
      <div className="px-6 pt-4 pb-4 border-b border-slate-50">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-[22px] font-bold text-[#001b4e]">
            Select Location
          </h2>
          {!isMandatory && onClose && (
            <button
      ...
      <div className="grow overflow-y-auto px-6 py-6 space-y-8">
        {/* GPS Button */}
        <button
          ...
          <div className="text-left">
            ...
            <div className="text-[12px] text-white/60">
              Using GPS for better precision
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 pb-8">
            {isSearching ? (
              <div className="flex flex-col items-center py-12 text-slate-400">
                <Loader2
                  size={32}
                  className="animate-spin mb-3 text-[#001b4e]"
                />
              ...
              (searchQuery.length >= 3 ? searchResults : popularCities).map(
                (city, idx) => (
                  <button
                    key={city.name + idx}
                    ...
                    <div className="text-left flex-1 min-w-0">
                      <div className="text-[15px] font-bold text-[#001b4e] truncate">
                        {city.name}
                      </div>
            ...
            {!isSearching &&
              ...
              searchResults.length === 0 && (
                <div className="text-center py-12 px-6 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  ...
                  <p className="text-[14px] font-bold text-[#001b4e]">
                    No cities found
                  </p>
